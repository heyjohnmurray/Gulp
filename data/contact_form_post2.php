<?
	...
	foreach( $_POST as $field => $value ) {
        if (is_scalar($_POST[$field])) {
    		$_POST[$field] = filter_var(htmlentities($_POST[$field]), FILTER_SANITIZE_STRING);
        }
	}

	$_SESSION['ContactArray'] = $_POST;

    // If null bytes or newlines in the referrer, send them home
    if (!RV_WebTools::is_valid_location($ref)) {
        $ref = '/';
    }
