<?
	...
	if($action == 'create_lead') {
		...
		$yearBuilt = (isset($_POST['YearBuilt']) ? $_POST['YearBuilt'] : '2000');
		
		// Check if serviceable
		try {
			$serviceable = HI_ServiceableHome::is_home_serviceable($_POST['ZipCode'], $yearBuilt, REFERAL_PARTNERS_SERVICEABLE);
			if(!$serviceable) {
				header("Location: /rd-quotes/home.php?zip=" . r3e($_POST['ZipCode']) . "&year=" . r3e($yearBuilt));
				die();
			}
		} catch(Exception $e) {}
		
		$_SESSION['contact_us_form_rmsg'] = "";
...
//        $FormValidator->attach_required_field('City', new RV_Validator_Address());
        ...
        try {
        	...
        	$_SESSION['contact_us_bad_fields'] = $FormValidator->get_failing_fields();

        }
        
    	// if we are using the clickclear form (values inside of input already, not label)
    	// this to adds the specified fields to the contact_us_bad_fields array so that it will not submit
    	// while the values are 'First Name' and 'Last Name' for example

    	if ($_POST['FirstName'] == 'First Name') {

	    		$_SESSION['contact_us_bad_fields'][] = 'FirstName';

    	}

    	if ($_POST['LastName'] == 'Last Name') {

	    		$_SESSION['contact_us_bad_fields'][] = 'LastName';

    	}
    	
    	// if we have bad fields, set error message and redirect back to page where form failed
    	if ( !empty($_SESSION['contact_us_bad_fields']) ) {
    	
	    	$_SESSION['contact_us_form_rmsg'] = "Please fix the highlighted fields";
        ...
        if(!$leadObj->process_lead($leadArray)) { // failure :(
            ...
            $_SESSION['contact_us_form_rmsg'] 					= $leadObj->errorText;
